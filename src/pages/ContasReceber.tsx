...
import { 
  ...
  Calendar, User, Tag, CreditCard, Info, Filter, Users
} from 'lucide-react';
...
export default function ContasReceber() {
  ...
  const [searchDevedor, setSearchDevedor] = useState('');
  ...
  function getVisualStatus(c: Conta) {
    if (c.status === 'recebido') return 'opacity-60';
    if (!c.data_vencimento) return '';
    const venc = parseISO(c.data_vencimento);
    const diff = differenceInDays(venc, today);

    if (diff < 0) return 'animate-pulse bg-destructive/10 border-destructive/50 text-destructive'; 
    if (diff === 0) return 'animate-pulse bg-yellow-500/10 border-yellow-500/50 text-yellow-600 dark:text-yellow-400'; 
    if (diff === 1) return 'bg-yellow-500/5 border-yellow-500/30 text-yellow-600 dark:text-yellow-400'; 
    if (diff === 2) return 'bg-orange-500/5 border-orange-500/30 text-orange-600 dark:text-orange-400'; 
    return '';
  }

  const filtered = contas.filter((c) => {
    const matchSearch = c.descricao?.toLowerCase().includes(search.toLowerCase()) || 
                        c.devedor_id?.toLowerCase().includes(search.toLowerCase());
    const matchStatus = filterStatus === 'todos' || 
                        (filterStatus === 'vencido' ? (parseISO(c.data_vencimento) < today && c.status === 'receber') : c.status === filterStatus);
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 bg-background min-h-screen text-foreground transition-colors">
      {/* Cabeçalho */}
      ...
      {/* Filtros */}
      <div className="bg-card border border-border p-4 rounded-2xl flex flex-wrap gap-3 shadow-sm">
        <div className="flex-1 min-w-[200px] relative">
          <Search className="absolute left-3 top-2.5 h-4 w-4 text-muted-foreground" />
          <input placeholder="Buscar por descrição ou devedor..." value={search} onChange={e => setSearch(e.target.value)}
      ...
      <div className="bg-card border border-border rounded-2xl overflow-hidden shadow-sm">
        <table className="w-full text-left text-xs">
          <thead>
            <tr className="bg-muted/50 text-muted-foreground uppercase text-[9px] font-black tracking-widest border-b border-border">
          ...
          <tbody className="divide-y divide-border">
            {filtered.map((c) => (
              <tr key={c.id} className={`group hover:bg-muted/30 transition-colors cursor-pointer ${getVisualStatus(c)}`} onClick={() => setViewing(c)}>
                ...
                <td className="p-4">
                  <span className="bg-secondary px-2 py-0.5 rounded text-[10px] uppercase font-black text-muted-foreground">{c.categoria || 'Geral'}</span>
                </td>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-md p-4">
          <div className="w-full max-w-xl bg-card border border-border rounded-3xl p-6 shadow-2xl overflow-y-auto max-h-[95vh]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2 space-y-3 p-4 bg-muted/30 rounded-2xl border border-border">
                ...
                <div className="flex gap-4">
                  {['cliente', 'fornecedor', 'manual'].map((t) => (
                    <label key={t} className="flex items-center gap-2 cursor-pointer group">
                      <input type="radio" checked={tipoDevedor === t} onChange={() => { setTipoDevedor(t as any); setForm({...form, devedor_id: ''}); }} className="accent-primary" />
                ...
                {tipoDevedor === 'manual' ? (
                  ...
                  <div className="relative">
                    ...
                    <input list="lista-pessoas" placeholder={`Buscar ${tipoDevedor}...`} value={form.devedor_id} onChange={e => setForm({...form, devedor_id: e.target.value})}
                      className="w-full bg-secondary border border-border rounded-xl py-2 pl-10 pr-4 text-sm outline-none" />
                    <datalist id="lista-pessoas">
                      {(tipoDevedor === 'cliente' ? clientes : fornecedores).map(p => <option key={p.id} value={p.nome} />)}
                    </datalist>
                  </div>
                )}
              </div>
