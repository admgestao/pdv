...
import { Package, Plus, Search, Pencil, Trash2, X, Save, Camera } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    if (scanning) {
      ...
      scanner.render((decodedText) => {
        ...
        // Erros de leitura silenciosos para não travar a câmera
      });
    }
            ...
            <table className="w-full text-sm text-left">
              <thead className="bg-zinc-900 text-zinc-400 border-b border-zinc-800">
                <tr>
                  ...
                  <th className="p-3 font-medium hidden md:table-cell">Código</th>
              ...
              <tbody>
                {filtered.map((p) => (
                  <tr key={p.id} className="border-b border-zinc-800 hover:bg-white/5 transition-colors">
                    <td className="p-3 font-medium">{p.nome}</td>
                    <td className="p-3 text-zinc-500 font-mono hidden md:table-cell">{p.codigo}</td>
                    <td className="p-3 text-right font-mono">{p.estoque_atual}</td>
                    <td className="p-3 text-right font-mono text-green-400">R$ {Number(p.preco_venda).toFixed(2)}</td>
