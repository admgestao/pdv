...
import { 
  TrendingDown, Plus, Search, Pencil, Trash2, X, Save, Check, 
  Calendar, User, Activity, Info, AlertTriangle
} from 'lucide-react';
...
export default function ContasPagar() {
  ...
  const filtered = contas.filter((c) => {
    ...
    const matchSearch = c.descricao?.toLowerCase().includes(search.toLowerCase()) || 
                        c.credor_id?.toLowerCase().includes(search.toLowerCase());
  ...
  function getVisualStatus(c: Conta) {
    if (c.status === 'pago') return 'opacity-60';
      ...
      <div className="bg-card border border-border rounded-2xl overflow-hidden shadow-sm">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead>
              <tr className="bg-muted/50 text-muted-foreground uppercase text-[9px] font-bold tracking-widest border-b border-border">
            ...
            <tbody className="divide-y divide-border">
              {filtered.map((c) => (
                <tr key={c.id} className={`group hover:bg-muted/30 transition-colors cursor-pointer ${getVisualStatus(c)}`}>
                  ...
                  <td className="p-4" onClick={() => setViewing(c)}>
                    <span className="bg-secondary px-2 py-1 rounded text-[10px] uppercase font-bold">{c.categoria || 'Geral'}</span>
                  </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm p-4">
          <div className="w-full max-w-lg bg-card border border-border rounded-3xl p-8 shadow-2xl relative">
            ...
            <div className="grid grid-cols-2 gap-4 border-t border-border pt-6">
              ...
              <div><p className="text-[9px] font-bold text-muted-foreground uppercase">Pagamento</p><p className="font-semibold">{viewing.condicao_pagamento || '—'}</p></div>
