...
interface CustomUser {
  ...
  nome_usuario: string;
  senha: string;
  nome_completo: string; // Atualizado aqui
...
export default function Usuarios() {
  ...
  const emptyUser: CustomUser = { id: '', nome_usuario: '', senha: '', nome_completo: '', cargo: 'operador', permissoes: [] };
  ...
  async function handleDelete(id: string) {
    ...
    logAction(user?.id || '', 'usuario_excluido', `Usuário excluído`);
    toast.success('Usuário excluído');
  }

  function togglePermission(key: string) {
    if (!editing) return;
    const perms = editing.permissoes.includes(key)
      ? editing.permissoes.filter(p => p !== key)
      : [...editing.permissoes, key];
  ...
  async function handleSave() {
    ...
    const { data: existing } = await supabase
      .from('usuarios')
      .select('id')
      .eq('nome_usuario', editing.nome_usuario)
      .neq('id', editing.id)
      .single();
    ...
    const { error } = await supabase.from('usuarios').upsert(editing);
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center border-b pb-2">
                ...
                <Button variant="ghost" size="sm" onClick={toggleAll} className="text-xs">Alternar Todos</Button>
              </div>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {ALL_MENUS.map(m => (
                  <div key={m.key} className="flex items-center gap-2 border p-2 rounded-md bg-muted/30 hover:bg-accent/50 transition-colors">
                    <Checkbox checked={editing.permissoes.includes(m.key)} onCheckedChange={() => togglePermission(m.key)} id={m.key} />
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button onClick={handleSave}><Save className="h-4 w-4 mr-2" /> Gravar Usuário</Button>
            </div>
          </CardContent>
        </Card>
      )}
                ...
                {users.map(u => (
                  <TableRow key={u.id}>
                    ...
                    <TableCell className="text-right flex justify-end gap-2">
                      <Button variant="ghost" size="icon" onClick={() => handleEdit(u)}><Edit2 className="h-4 w-4" /></Button>
